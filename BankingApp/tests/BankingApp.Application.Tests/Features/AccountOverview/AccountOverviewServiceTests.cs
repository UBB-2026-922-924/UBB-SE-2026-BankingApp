...
public sealed class AccountOverviewServiceTests
{
    ...
    private readonly AccountOverviewService accountOverviewService;
    public AccountOverviewServiceTests()
    {
        mockUserRepository = MockFactory.CreateUserRepositoryMock();
        mockAccountRepository = MockFactory.CreateAccountRepositoryMock();
        mockTransactionRepository = MockFactory.CreateTransactionRepositoryMock();
    ...
    {
        mockUserRepository.Setup(r => r.GetByIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((User)null!);

        ErrorOr<AccountOverviewDto> getDashboardResponse = await accountOverviewService.GetDashboardAsync(1, TestContext.Current.CancellationToken);
    ...
    public async Task GetDashboard_WhenUserExists_ReturnsTransactions()
    {
        ...
        mockAccountRepository.Setup(r => r.ListByUserIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
        ...
        mockTransactionRepository.Setup(r => r.ListByAccountIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>())).ReturnsAsync(new ReadOnlyCollection<Domain.Aggregates.AccountAggregate.Entities.Transaction>(new List<Transaction>([transaction])));
